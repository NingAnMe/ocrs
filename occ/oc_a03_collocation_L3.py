...
from app.pb_drc_MERSI_L3 import CLASS_MERSI_L3
from app.pb_drc_MODIS_L3 import CLASS_MODIS_L3
import numpy as np
...
#             print homoValue1[ccc1]
    ...
    def write_hdf5(self, ICFG, MCFG):
        ...
        ofile2 = ICFG.ofile.split('.')[0] + '.HDF5'
...
#             vmax = 10.
#             p.easyplot(
#                 self.MERSI_Lats, self.MERSI_Lons, self.MERSI_FovMean[Band], vmin=vmin, vmax=vmax, markersize=5, marker='s')
#             p.title = u'mersi'
#             ofile = os.path.join(MainPath, 'MERSI_%s.png' % Band)
#             p.savefig(ofile)
#
#         for Band in MCFG.chan1:
#             p = dv_map()
...
#             vmax = 10.
#             p.easyplot(
#                 self.MODIS_Lats, self.MODIS_Lons, self.MODIS_FovMean[Band], vmin=vmin, vmax=vmax, markersize=5, marker='s')
#             p.title = u'modis'
#             ofile = os.path.join(MainPath, 'MODIS_%s.png' % Band)
#             p.savefig(ofile)
...
def main(inYamlFile):
    ...
    modeFile = os.path.join(
        MainPath, 'cfg', '%s+%s_%s+%s_L3.colloc' % (ICFG.sat1, ICFG.sensor1, ICFG.sat2, ICFG.sensor2))
    ...
    P1 = prj_core(
        ICFG.cmd, ICFG.res, unit='deg', pt_tl=(-179.5, 89.5), pt_br=(179.5, -89.5))
    P2 = prj_core(
        ICFG.cmd, ICFG.res, unit='deg', pt_tl=(-179.5, 89.5), pt_br=(179.5, -89.5))
